...
private:
	...
	boost::function<void()> m_disconnect_callback;

protected:
	NetworkConnection(boost::shared_ptr< NetworkService > service);
	virtual ~NetworkConnection();

private:
	NetworkConnection(const NetworkConnection & rhs);
	NetworkConnection & operator =(const NetworkConnection & rhs);
	void StartSend();
